...
- `AuditLog` 操作审计链：危险工具调用按天写 JSONL 到 `~/.yicli/audit/`，含 `outcome (allow|deny|error)` 与 `approver (hitl|policy|none)`
...
- YiCLI 的安全模型是 **HITL + 路径校验 + 命令快速拒绝 + 审计**，不是隔离
...
- 配置持久化：`~/.yicli/config.json` 存储默认模型，支持 `.env` 回退读取 API Key
...
**目标**：把 YiCLI 接入 MCP 生态。stdio 子进程 server 与 Streamable HTTP 远程 server 都能用，工具自动注册到 ToolRegistry，与 HITL / AuditLog 协同。
...
- 配置文件：`~/.yicli/mcp.json`（用户级）+ `.yicli/mcp.json`（项目级，可入 git），格式与 Claude Code `claude_desktop_config.json` 兼容
...
- **默认 enabled**：`~/.yicli/mcp.json` 不存在时启动自动创建模板，含 chrome-devtools 条目
...
- `McpClient.initialize` 超时 30s → 60s（chrome-devtools 首次启动需 npx 拉包 + Chrome 冷启 ≈ 20s+），可被 `yicli.mcp.initialize.timeout.seconds` 覆盖
...
**目标**：做出 YiCLI 自己的 Skill 加载机制，把零散的工具与决策指引打包成可复用单元，并以 web-access 作为首个落地 Skill

**功能迭代**（详细开发任务见 `docs/phase-15-skill-system.md`）：
- Skill 加载机制：三层目录扫描（jar 内置 / 用户级 `~/.yicli/skills/` / 项目级 `<project>/.yicli/skills/`），按 name 整体覆盖，frontmatter 走手写 YAML 子集解析（不引 SnakeYAML）
...
- 启动期 `SkillBuiltinExtractor` 把 jar 内置 skill 解压到 `~/.yicli/skills-cache/`，按 `.version` 文件控制重建
...
- 文件树浏览
- 代码高亮显示
- 对话历史可视化（`~/.yicli/history/session_*.jsonl`）
...
- 切换：`YICLI_RENDERER=inline|lanterna|plain`，旧 `YICLI_TUI=true` 兼容映射到 lanterna
...
**设计参考**：DeepSeek TUI `crates/tui/src/lsp/`——`LspManager`（惰性 transport pool）+ `lsp_hooks.rs`（post-edit 挂钩）+ `diagnostics.rs`（诊断类型与渲染）。YiCLI 的 Java 生态可以用 Eclipse JDT LS（`org.eclipse.jdt.ls`）或直接复用已有的 `CodeAnalyzer` 做轻量版。
...
**功能迭代**：
- `SideGitManager`：在 `~/.yicli/snapshots/<project_hash>/<worktree_hash>/.git` 维护独立 side-git 仓库，通过 JGit 纯 Java 实现，与用户的工作区 `.git` 完全隔离
...
**当前状态**：MVP 已落地。ReAct、Plan task executor、Multi-Agent 三角色、Planner 已接入 `PromptAssembler`，内置资源位于 `src/main/resources/prompts/`，覆盖路径支持 `~/.yicli/prompts/...` 与 `.yicli/prompts/...`。
...
- 用户级覆盖：`~/.yicli/prompts/base.md` 可整体替换内置 base.md；`~/.yicli/prompts/modes/agent.md` 可覆盖特定模式；项目级 `.yicli/prompts/...` 优先级更高
...
**目标**：用户可以在 TUI 里提交后台任务（如"重构整个模块"），关掉终端走人，回来查看结果。同时暴露 HTTP/SSE Runtime API，让 YiCLI 可以嵌入 CI/CD、IDE 插件、Web 面板。
...
**Runtime API**：
- `RuntimeApiServer`：嵌入式 HTTP/SSE 服务端（`yicli serve --http --port 8080`），基于已有的 OkHttp / Javalin 或 Spring Boot 内嵌
...
**当前 MVP 已落地**：
- `DurableTaskManager`：SQLite 后台任务队列，默认 `~/.yicli/tasks/tasks.db`
- `/task`、`/task add`、`/task cancel`、`/task log` CLI 闭环
- 进程启动时将残留 `running` 任务恢复为 `enqueued`
- Worker Pool 默认 2，可用 `YICLI_TASK_WORKERS` / `-Dyicli.task.workers` 覆盖
- `RuntimeApiServer`：基于 JDK `HttpServer`，仅监听 `127.0.0.1`
- `RuntimeThreadStore`：SQLite 保存 thread 与 event 时间线
- Runtime API 强制 `YICLI_RUNTIME_API_KEY` / `-Dyicli.runtime.api.key`
...
**目标**：让 YiCLI 真正"看见"图片——浏览器截图、用户粘贴的图片、文档中的图表，都能直接喂给 LLM 让它理解，而不是 fallback 成"[此工具返回了 image]"占位。此期排在安全网（LSP + 快照）和架构重构（Prompt + Task）之后，确保模型生态成熟时再做。
...
*已完成第 16 期 TUI 产品化（含 16.1 形态修正：默认切换为 inline 流式 TUI，Lanterna 全屏 TUI 通过 `YICLI_RENDERER=lanterna` 保留）、第 17 期 LSP 诊断注入 MVP、第 18 期 Git Side-History 快照与回滚 MVP、第 19 期 Prompt 分层架构 MVP、第 20 期后台任务 + Runtime API MVP、第 21 期图片复制粘贴输入 MVP。*
