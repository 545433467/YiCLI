...
- `PAI.md` 是 YiCLI 的项目级记忆文件：启动时自动注入 system prompt，适合团队共享的长期稳定规则；个人/会变化的经验继续用 `/save` 长期记忆。
- 下一步：OAuth / sampling / recovery 作为后续 MCP 增强
- Banner 版本：`v16.1.0`，Maven 产物：`yicli-1.0-SNAPSHOT.jar`（两者不一致是正常状态）
...
/sessions                # 列出已保存的历史会话（退出时自动保存）
/resume <id>             # 恢复历史会话；取消/失败轮次会自动保存检查点
...
核心内置工具 17 个：`read_file` / `write_file` / `list_dir` / `glob_files` / `grep_code` / `execute_command` / `create_project` / `search_code` / `web_search` / `web_fetch` / `browser_connect` / `browser_disconnect` / `browser_status` / `load_skill` / `save_memory` / `revert_turn` / `task`（`task` 会把独立探索任务分发给子代理并行执行，嵌套最多 2 层）

进程内事件总线 `YiCliEventBus` 提供 `tool_call_started/completed/failed`、`turn_started/ended`、`app_stop` 生命周期事件，供 hooks / 可观测性订阅；事件总线只做观察者通知，HITL / 策略层仍在调用链内同步拦截。批准过的危险工具调用会记入 `~/.yicli/permissions.json`（按 tool + 归一化参数精确匹配），`/permission` 可查看与清空。
...
MCP 配置会合并用户级 `~/.yicli/mcp.json` 与项目级 `.yicli/mcp.json`；`${VAR}` 支持系统环境变量、系统属性、项目 `.env`、用户 `~/.env`。检测到 `STEP_API_KEY` 时会自动内置 `step_search` 远程 MCP（显式同名配置优先）。
...
讯飞星辰 MaaS provider 名为 `xfyun`，默认 Base URL 为 `https://maas-api.cn-huabei-1.xf-yun.com/v2`。`model` 必须使用服务管控页展示的 `modelId`；公开模型名 / Hugging Face 仓库名不一定可直接调用。微调模型用 `/config provider xfyun --lora-id <resourceId>` 配置服务卡片上的 resourceId，YiCLI 会作为 HTTP header `lora_id` 发出。`xfyun` 当前按 MaaS 文档走纯对话请求，不向上游发送 YiCLI 内置工具列表。
...
- 启动期 MCP 不得阻塞首屏：CLI 默认最多等待 8 秒（`YICLI_MCP_STARTUP_WAIT_SECONDS` / `-Dyicli.mcp.startup.wait.seconds` 可调），超时后保留未完成 server 为 `STARTING` 并后台继续初始化；`/mcp` 查看最新状态。
- `LineReader` 使用 `YiCliHighlighter` 做输入实时高亮：slash 命令、`@` 引用、`@image:`、`@clipboard`、敏感词和明显危险 shell 片段会在编辑阶段被标记；不要把这类视觉提示混入最终提交文本。
- `LineReader` 使用 `YiCliCompleter` 做上下文补全：`/model` provider、`/mcp` 子命令与 server、`/skill` 子命令与 skill name、`/task` / `/browser` / `/snapshot` 子命令、`@image:` 本地路径、本地 `@path` 和 MCP resource `@server:uri` 引用都应从同一个 completer 出口维护。
...
- `LineReader` 使用 `YiCliHistory` 持久化输入历史到 `~/.yicli/history/input.history`；如果 `yicli.history.file` / `YICLI_HISTORY_FILE` 指向目录，也会自动使用该目录下的 `input.history`，避免把目录当文件读；默认忽略空白、重复、明显密钥/Bearer、base64 图片和超长输入，用户可用 `/history clear` 清空本机输入历史。
- 启动期会加载 `~/.yicli/PAI.md`、项目根 `PAI.md`、项目根 `.yicli/PAI.md`、`PAI.local.md`、`.yicli/PAI.local.md`，按此顺序注入 Project Context；`@relative/path.md` 可导入项目根内文件，总注入内容有字符预算，避免项目记忆变成 token 噪音。
- `/init` 会根据当前项目生成短 `PAI.md`，只放 commands / project positioning / architecture / pitfalls / don'ts；默认不覆盖已有文件。
- `/export` 导出当前 ReAct `conversationHistory` 为 Markdown 到 `~/.yicli/exports/session-*.md`；只支持无参数命令，包含完整 system prompt，便于检查 LLM 实际接收前的指令。
