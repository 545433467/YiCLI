...
import com.yicli.llm.LlmTraceLogger;
import com.yicli.context.ContextProfile;
import com.yicli.context.TokenUsageFormatter;
import com.yicli.lsp.LspDiagnosticReport;
import com.yicli.memory.ConversationHistoryCompactor;
import com.yicli.memory.ExplicitMemoryHints;
import com.yicli.memory.MemoryManager;
import com.yicli.prompt.PromptAssembler;
import com.yicli.prompt.ToolCatalogFormatter;
import com.yicli.prompt.PromptContext;
import com.yicli.prompt.PromptMode;
import com.yicli.prompt.ProjectMemoryLoader;
import com.yicli.render.PlainRenderer;
import com.yicli.render.Renderer;
import com.yicli.render.StatusInfo;
import com.yicli.runtime.CancellationContext;
import com.yicli.skill.SkillContextBuffer;
import com.yicli.skill.SkillIndexFormatter;
import com.yicli.skill.SkillRegistry;
import com.yicli.util.AnsiStyle;
import com.yicli.tool.ToolRegistry;
import com.yicli.tool.ToolRegistry.ToolExecutionResult;
import com.yicli.tool.ToolRegistry.ToolInvocation;
import com.yicli.util.TerminalMarkdownRenderer;
import com.yicli.image.ImageReferenceParser;
...
public class Agent {
    ...
    @FunctionalInterface
    public interface SessionCheckpointer {
        String save(List<LlmClient.Message> history, String title) throws Exception;
    }
    ...
    public void setEventBus(YiCliEventBus eventBus) {
        this.eventBus = eventBus;
    }

    public void setSessionCheckpointer(SessionCheckpointer sessionCheckpointer) {
        this.sessionCheckpointer = sessionCheckpointer;
    }
    ...
    public String run(String userInput) {
        ...
        publishTurnStarted();
        // try/finally 只包裹主循环：无论正常结束、budget 耗尽、取消还是 IO 失败，
        // 都会发出 TURN_ENDED，供事件总线订阅者（hooks / 可观测性）收尾。
        try {
        ...
        while (true) {
            if (CancellationContext.isCancelled()) {
                ...
                return "⏹️ 已取消当前任务。" + checkpointSuffix();
            }
            ...
            try {
                ...
                streamRenderer.finish();
                return "❌ 调用 LLM 失败: " + e.getMessage() + checkpointSuffix();
            }
        }
        } finally {
    ...
    private void publishTurnStarted() {
        if (eventBus != null) {
            eventBus.publish(YiCliEvent.plain(YiCliEvent.TURN_STARTED));
        }
    }
    ...
    /**
     * 取消 / 失败时把当前对话历史保存为检查点，返回可续跑提示。
     * 保存失败只记日志，不影响主流程返回。
     */
    private String checkpointSuffix() {
        if (sessionCheckpointer == null) {
            return "";
        }
        try {
            String id = sessionCheckpointer.save(conversationHistory, sessionTitle());
            lastCheckpointId = id;
            return "\n💾 已保存检查点: " + id + "，可用 /resume " + id + " 继续。";
        } catch (Exception e) {
    ...
    /**
     * 恢复持久化的会话历史（/resume）。首条若已是 system 则原样保留，
     * 否则重建一份带当前记忆上下文的 system prompt 前置，避免历史缺 system 段。
     */
    public void restoreConversationHistory(List<LlmClient.Message> history) {
        conversationHistory.clear();
        ...
        if (restored.isEmpty() || !"system".equals(restored.get(0).role())) {
            conversationHistory.add(LlmClient.Message.system(buildSystemPrompt("")));
        }
    ...
    /** 会话标题：第一条 user 消息的前 60 字符，用于 /sessions 列表展示。 */
    public String sessionTitle() {
        for (LlmClient.Message msg : conversationHistory) {
            if (msg != null && "user".equals(msg.role())) {
                String text = msg.content() == null ? "" : msg.content().trim().replace('\n', ' ');
                if (text.length() > 60) {
    ...
    private String buildSystemPrompt(String memoryContext) {
        return promptAssembler.assemble(PromptMode.AGENT, PromptContext.builder()
                ...
                .toolCatalog(ToolCatalogFormatter.format(toolRegistry.getToolDefinitions()))
                .build());
    }
    ...
    public String getContextStatus() {
        com.yicli.context.ContextProfile profile = memoryManager.getContextProfile();
        ...
        for (LlmClient.Message msg : conversationHistory) {
            int t = com.yicli.memory.TokenBudget.estimateMessagesTokens(java.util.List.of(msg));
            switch (msg.role()) {
    ...
    private int estimateToolsSchemaTokens() {
        try {
            return com.yicli.memory.MemoryEntry.estimateTokens(
    ...
    private long estimateCurrentContextTokens() {
        long messageTokens = com.yicli.memory.TokenBudget.estimateMessagesTokens(conversationHistory);
    ...
    private void logRequestContext(String scope, List<LlmClient.Tool> tools) {
        ...
        for (int messageIndex = 0; messageIndex < conversationHistory.size(); messageIndex++) {
            ...
            messages++;
            int tokens = com.yicli.memory.TokenBudget.estimateMessagesTokens(List.of(msg));
        ...
        if (tools != null && !tools.isEmpty()) {
            try {
                toolsSchemaTokens = com.yicli.memory.MemoryEntry.estimateTokens(
    ...
    private static final class StreamRenderer implements LlmClient.StreamListener {
        ...
        public void onRetry(int attempt, long delayMillis, String reason) {
            // live thinking 面板重绘时会清理自己刚打印的行，普通输出会干扰面板；
            // 因此只在非面板路径提示重试，面板路径靠底部状态与日志反映。
            if (hasThinkingPanel()) {
                return;
            }
            out().println(AnsiStyle.subtle(
                    "  ↻ LLM 调用失败，正在重试（第 " + attempt + " 次），原因: " + reason
                            + "，等待 " + Math.max(1, delayMillis / 1000) + "s"));
            out().flush();
        }
