 ...
 * 1. 系统属性：{@code yicli.react.token.budget} / {@code yicli.react.stagnation.window} /
 *    {@code yicli.react.hard.max.iterations}
 ...
 * 场景（CI / 自动化批跑）通过 {@code -Dyicli.react.token.budget=N} 显式启用。
...
public class AgentBudget {
    ...
    public static AgentBudget fromLlmClient(LlmClient llmClient) {
        ...
        // 显式 -Dyicli.react.token.budget=N 仍可启用硬预算，覆盖默认。
        return new AgentBudget(
                readIntProperty("yicli.react.token.budget", Integer.MAX_VALUE),
                readIntProperty("yicli.react.stagnation.window", DEFAULT_STAGNATION_WINDOW),
                readIntProperty("yicli.react.hard.max.iterations", DEFAULT_HARD_MAX_ITERATIONS)
        );
    }
