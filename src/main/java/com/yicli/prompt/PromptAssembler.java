...
public class PromptAssembler {
    ...
    public String assemble(PromptMode mode, PromptContext context) {
        ...
        if (ctx.toolCatalog() != null && !ctx.toolCatalog().isBlank()) {
            base = replaceToolCatalog(base, ctx.toolCatalog());
        }
        if (!ctx.toolsEnabled()) {
    ...
    /**
     * 用 ToolRegistry 动态生成的工具清单替换 base.md 中静态的 "## Tools" 段，
     * 避免模型看到的能力描述与实际注册工具漂移。替换失败时保留静态清单兜底。
     */
    private static String replaceToolCatalog(String base, String catalog) {
        String generated = "## Tools\n\n" + catalog.trim();
        String replaced = base.replaceFirst("(?s)## Tools\\n.*?(?=\\n## )", generated);
        return replaced.equals(base) ? base : replaced;
    }
    ...
    private static String noToolsSection() {
        return """
                ...
                当前模型不支持 YiCLI 原生工具调用。本轮不要声称已经读取、搜索、执行或修改了任何本地文件、命令、浏览器、MCP resource 或外部工具结果。
