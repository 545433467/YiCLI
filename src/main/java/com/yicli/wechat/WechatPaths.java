...
public final class WechatPaths {
    ...
    public static Path root() {
        String configured = System.getProperty("yicli.wechat.dir");
        if (configured == null || configured.isBlank()) {
            configured = System.getenv("YICLI_WECHAT_DIR");
        }
        if (configured == null || configured.isBlank()) {
            configured = Path.of(System.getProperty("user.home"), ".yicli", "wechat").toString();
        }
    ...
    public static Path pidFile() {
        return root().resolve("yicli-wechat.pid");
    }
}
