...
import com.yicli.render.inline.TerminalCapabilities;
 ...
 * <p>选型规则：
 * <ul>
 *   <li>{@code -Dyicli.renderer} > {@code YICLI_RENDERER} 环境变量 > 默认 inline</li>
 ...
 *   <li>兼容：{@code YICLI_TUI=true} → 等价 {@code lanterna}，打 deprecation 提示</li>
 * </ul>
 *
...
public final class RendererFactory {
    ...
    public static Mode resolveMode() {
        ...
        // 兼容旧 YICLI_TUI=true → lanterna
        String legacyTui = System.getenv("YICLI_TUI");
        if (legacyTui != null && Boolean.parseBoolean(legacyTui.trim())) {
            System.err.println("⚠️ YICLI_TUI=true 已废弃，请改用 YICLI_RENDERER=lanterna");
    ...
    private static Mode parse(String raw) {
        return switch (raw.trim().toLowerCase()) {
            ...
            default -> {
                System.err.println("⚠️ 未识别的 YICLI_RENDERER='" + raw + "'，回退到 inline");
                yield Mode.INLINE;
            }
        };
    }
