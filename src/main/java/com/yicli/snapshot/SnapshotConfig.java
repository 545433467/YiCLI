...
public record SnapshotConfig(
    ...
    public static SnapshotConfig fromEnvironment() {
        boolean enabled = readBoolean("yicli.snapshot.enabled", "YICLI_SNAPSHOT_ENABLED", true);
        Path root = Path.of(readString("yicli.snapshot.dir", "YICLI_SNAPSHOT_DIR",
                Path.of(System.getProperty("user.home"), ".yicli", "snapshots").toString()));
        int max = readInt("yicli.snapshot.max", "YICLI_SNAPSHOT_MAX", 50);
        List<String> excludes = mergeExcludes(readString("yicli.snapshot.excludes", "YICLI_SNAPSHOT_EXCLUDES", ""));
