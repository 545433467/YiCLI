...
final class CliCommandParser {
    ...
    static ParsedCommand parse(String input) {
        ...
        if (trimmed.equalsIgnoreCase("/permission clear")) {
            return new ParsedCommand(CommandType.PERMISSION_CLEAR, null);
        }

        if (trimmed.equalsIgnoreCase("/permission")) {
            return new ParsedCommand(CommandType.PERMISSION, null);
        }
