...
public interface LlmClient {
    ...
    interface StreamListener {
        ...
        /**
         * LLM 调用遇到可重试失败（429 / 5xx / 首包前的连接错误）时回调。
         * 已在调用方展示输出流后不会触发（避免重复内容）。
         *
         * @param attempt    已失败的尝试序号（从 1 开始）
         * @param delayMillis 下次重试前的等待毫秒数
         * @param reason     失败原因摘要
         */
        default void onRetry(int attempt, long delayMillis, String reason) {}
    }
