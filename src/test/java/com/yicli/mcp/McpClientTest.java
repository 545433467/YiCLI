...
class McpClientTest {
    ...
    void initializeTimeoutCanBeOverriddenBySystemProperty() {
        String previous = System.getProperty("yicli.mcp.initialize.timeout.seconds");
        try {
            System.setProperty("yicli.mcp.initialize.timeout.seconds", "17");
            assertEquals(17, McpClient.initializeTimeoutSeconds());
        } finally {
            if (previous == null) {
                System.clearProperty("yicli.mcp.initialize.timeout.seconds");
            } else {
                System.setProperty("yicli.mcp.initialize.timeout.seconds", previous);
            }
        }
    }
    ...
    void closeIsBestEffortAndDoesNotBlockOnServerSilence() throws Exception {
        // close 不再发 shutdown notification（server 卡死时会让 YiCLI 退出阻塞）。
