...
class McpToolRegistrationTest {
    ...
    private static void withAuditDir(Path tempDir, ThrowingRunnable body) throws Exception {
        String previous = System.getProperty("yicli.audit.dir");
        System.setProperty("yicli.audit.dir", tempDir.resolve("audit").toString());
        try {
            body.run();
        } finally {
            if (previous == null) {
                System.clearProperty("yicli.audit.dir");
            } else {
                System.setProperty("yicli.audit.dir", previous);
            }
        }
    }
