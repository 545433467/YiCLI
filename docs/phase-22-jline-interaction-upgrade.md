...
本阶段目标：把 YiCLI 从“使用 JLine 的 CLI”升级为“以 JLine 4 为核心 UI runtime 的 Agent 终端”，交互体验对齐 Claude Code / Qoder CLI 一类产品。
...
- 增加 `YiCliHighlighter`：高亮 `/command`、`@path`、`@image:`、`@clipboard`。
- 对危险命令关键词做视觉提示。
- 对未闭合括号/引号做轻量提示。
...
- 已扩展 `YiCliCompleter` 的上下文补全入口，避免多个输入态各自维护补全逻辑。
...
- 已接入 `YiCliHistory`，基于 JLine `DefaultHistory` 做 YiCLI 过滤策略。
- 默认配置 `~/.yicli/history/input.history` 持久化，可用 `yicli.history.file` / `YICLI_HISTORY_FILE` 覆盖；如果配置值指向目录，则使用该目录下的 `input.history`。
- 已设置 history size / file size，默认 `2000` / `10000`，可用 `yicli.history.size`、`yicli.history.fileSize` 或对应环境变量覆盖。
- 已忽略空白、重复、超长输入。
...
- 暂不使用 `jline-shell` 替换 YiCLI 现有命令解析。
...
- HITL 审批期间不吞输入。
- `java -jar target/yicli-1.0-SNAPSHOT.jar` 真机验证。
