...
- 新增 YiCLI 内置 WeChat Channel，默认关闭；只有显式执行 `yicli wechat ...` 才启动 iLink。
- 不做 Skill、不走 Runtime API；微信接收用 iLink `getupdates` long-poll，回复流式体验靠 `sendmessage` 分块。
- iLink 协议层出站回复仍是 `text_item.text` 文本消息，没有显式 Markdown parse mode；YiCLI 保留 ClawBot 稳定支持的 Markdown 子集，并对标题、表格和非代码 fenced block 做移动端友好的归一化。
...
- 新增 top-level 入口，必须在 LLM/API Key/JLine/MCP 初始化前分发：
  - `/wechat`：在 YiCLI 主交互内扫码确认绑定并在当前进程后台启动 long-poll；已绑定时直接启动。
  - `/wechat setup`：重新扫码绑定并启动。
  - `/wechat status` / `/wechat stop`：查看或关闭当前进程内通道。
  - `yicli wechat setup`：进程级扫码确认绑定，保存 token/baseUrl/botId/boundUserId/workspace/策略配置，不启动轮询。
  - `yicli wechat start`：进程级前台启动 long-poll，适合脚本 / daemon。
  - `yicli wechat daemon start|stop|restart|status|logs`：后台服务管理。
- 新增 `com.yicli.wechat` 结构：
  - `IlinkClient`：QR、`getupdates`、`sendmessage`、`sendtyping`、`getuploadurl`、`notifystart/notifystop`。
  - `WechatAccountStore`：`~/.yicli/wechat/` 下保存账号、session、sync buf、media、logs；敏感字段脱敏，文件权限 `600`。
  ...
  - `WechatAgentSession`：无 JLine 初始化，复用 YiCLI config、LLM、MCP、Skill、Memory、ToolRegistry；注入 `WechatPolicyDecider` 替代交互式 HITL handler。
- 新增微信专用交互层：
...
- 媒体规则：
  ...
  - **文件推送只走显式 `/send <path>` 或「本 turn 工具产物登记表」**；不再「检测到回复里出现的本地路径就自动上传」——避免把 `~/.ssh/id_rsa`、`~/.yicli/config.json` 等路径无意外发。推送路径同样受 workspace / 白名单约束。
...
- v1 不做群聊、语音、视频。
- 微信通道**不启用交互式 HITL，改用非交互式默认拒绝策略**；不代表绕过 YiCLI 策略层（PathGuard / CommandGuard / 审计 / 工作区边界全部保留）。
