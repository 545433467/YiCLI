...
让 YiCLI 能把图片作为真正的 LLM 输入，而不是只把 MCP 工具返回的 image content 格式化成占位文字。
...
- 不按模型名在 YiCLI 输入层拦截图片；如果 provider 不接受某个模型 / endpoint 的图片输入，应暴露 provider 的真实错误或走 provider 内部图片请求路由
