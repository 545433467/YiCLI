...
1. `~/.yicli/config.json` 中对应 provider 的 `apiKey`
...
| 数据 | 默认路径 | 覆盖方式 |
|------|----------|----------|
| 长期记忆 | `~/.yicli/memory/long_term_memory.json` | `-Dyicli.memory.dir` |
| 项目级记忆 | `PAI.md` / `.yicli/PAI.md` / `PAI.local.md` | 用户级稳定偏好：`~/.yicli/PAI.md` |
| RAG 索引 | `~/.yicli/rag/codebase.db` | `-Dyicli.rag.dir` |
| 审计日志 | `~/.yicli/audit/audit-YYYY-MM-DD.jsonl` | `YICLI_AUDIT_DIR` / `-Dyicli.audit.dir` |
| Side-Git 快照 | `~/.yicli/snapshots/<project_hash>/<worktree_hash>/.git` | `YICLI_SNAPSHOT_DIR` / `-Dyicli.snapshot.dir` |
| 后台任务 | `~/.yicli/tasks/tasks.db` | — |

### Snapshot Config

系统属性 > 环境变量 > 默认值：`yicli.snapshot.enabled`(true) / `yicli.snapshot.max`(50) / `yicli.snapshot.excludes`(.git,.yicli/snapshots,target,node_modules,dist,.idea,*.class,*.jar) / `yicli.snapshot.dir`(~/.yicli/snapshots)
...
系统属性 > 环境变量/.env > 默认值：`YICLI_LOG_DIR`(~/.yicli/logs) / `YICLI_LOG_LEVEL`(INFO) / `YICLI_LOG_MAX_HISTORY`(7) / `YICLI_LOG_MAX_FILE_SIZE`(10MB) / `YICLI_LOG_TOTAL_SIZE_CAP`(100MB)

### ReAct/SubAgent Budget Config

系统属性 > 默认值：`yicli.react.token.budget`(Integer.MAX_VALUE) / `yicli.react.stagnation.window`(3) / `yicli.react.hard.max.iterations`(50)
...
系统属性 > 默认值：`yicli.llm.connect.timeout.seconds`(60) / `yicli.llm.read.timeout.seconds`(300) / `yicli.llm.write.timeout.seconds`(60) / `yicli.llm.call.timeout.seconds`(600)
...
1. 用户级：`~/.yicli/mcp.json`
2. 项目级：`.yicli/mcp.json`
...
- 加载顺序：`~/.yicli/PAI.md` → `PAI.md` → `.yicli/PAI.md` → `PAI.local.md` → `.yicli/PAI.local.md`
...
- shared 模式 close_page 只允许关闭 YiCLI 创建的 tab

### Skill System

- 三层加载：jar 内置 < 用户级 ~/.yicli/skills/ < 项目级 .yicli/skills/
...
- 环境变量：`YICLI_RENDERER=inline|lanterna|plain`
- `YICLI_TUI=true`(旧) → lanterna + deprecation 提示
- `YICLI_NO_STATUSBAR=true`：禁用底部状态栏
...
- `/export` 导出当前 ReAct conversationHistory 为 Markdown 到 `~/.yicli/exports/session-*.md`；包含完整 system prompt，便于检查 LLM 实际接收前的指令，命令不接受路径参数。
...
- `YICLI_LSP_ENABLED=false` 关闭

### Git Side-History Snapshot (Phase 18)

- side-git 在 ~/.yicli/snapshots/ 维护独立仓库（JGit，不依赖系统 git）
...
- 覆盖优先级：jar 内置 < 用户级 ~/.yicli/prompts/ < 项目级 .yicli/prompts/
...
- XfyunMaaSClient：Qwen3.6-35B-A3B，默认 https://maas-api.cn-huabei-1.xf-yun.com/v2，OpenAI-compatible 讯飞星辰 MaaS；可用 `/config provider xfyun ...` 写入配置后 `/model xfyun` 切换。`model` 必须使用 MaaS 服务管控页展示的 modelId；微调模型可配置 `--lora-id <resourceId>`，作为 HTTP header `lora_id` 发出；该 provider 不发送 YiCLI 内置 tools。
...
# YICLI_LOG_LEVEL=INFO
# YICLI_LOG_DIR=/Users/yourname/.yicli/logs
# YICLI_LOG_MAX_HISTORY=7
# YICLI_LOG_MAX_FILE_SIZE=10MB
# YICLI_LOG_TOTAL_SIZE_CAP=100MB
# YICLI_SNAPSHOT_ENABLED=true
# YICLI_SNAPSHOT_MAX=50
# YICLI_SNAPSHOT_EXCLUDES=.git,.yicli/snapshots,target,node_modules,dist,.idea,*.class,*.jar
# YICLI_SNAPSHOT_DIR=/Users/yourname/.yicli/snapshots
# YICLI_TUI=true
# NO_TUI=true
```
