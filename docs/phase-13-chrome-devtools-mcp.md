...
> 4. `src/main/java/com/yicli/mcp/` 整套现有代码 —— 你只是在这套基础上增量配置 + 适配
> 5. `src/main/java/com/yicli/hitl/HitlToolRegistry.java`、`TerminalHitlHandler.java` —— HITL「server 维度全部放行」要改这两处
>
> **核心原则**：第 10/11 期已经做完 MCP 框架，本期**不写浏览器自动化逻辑**——直接接 Google 官方 `chrome-devtools-mcp` server，YiCLI 这边只做"接入配置 + 适配层 + UX 优化"。

---

## 1. 目标与产出物

让 YiCLI 能操控浏览器：处理 SPA / JS 渲染 / 防爬墙 / 表单交互 / 登录后页面。**直接接 Google 官方 `chrome-devtools-mcp@latest`**（v0.23.0+，2026 年 4 月仍在活跃维护，37k stars，28 个工具）。

最终交付：

- `~/.yicli/mcp.json` 默认包含 `chrome-devtools` server，**默认 enabled**
...
注册到 YiCLI 后命名为 `mcp__chrome-devtools__navigate_page` 等。
...
- 用户要看页面长什么样、UI 验收 → take_screenshot（截图无法直接给 LLM 看，由 YiCLI 附给用户）
...
YiCLI 启动时如果 `~/.yicli/mcp.json` 不存在，**自动创建**含 chrome-devtools 的最小模板。如果存在但缺 chrome-devtools 条目，启动时打印一行提示「检测到未配置 chrome-devtools，建议参考 README 添加」（不自动改用户文件）。
...
# YICLI_MCP_INITIALIZE_TIMEOUT_SECONDS=60
```
...
| 文件 | 改动 |
|---|---|
| `~/.yicli/mcp.json`（用户文件，启动时检测） | `Main.java` 启动时检测：文件不存在则创建默认含 chrome-devtools 的模板 |
| `src/main/java/com/yicli/mcp/McpClient.java` | initialize 超时 30s → 60s（或读 `yicli.mcp.initialize.timeout.seconds` 系统属性） |
| `src/main/java/com/yicli/mcp/McpServerManager.java` | startAll 期间另起 status printer 线程，每 5s 打印未就绪 server 的等待时长 |
| `src/main/java/com/yicli/hitl/TerminalHitlHandler.java` | `approvedAllTools` 拆成 `approvedAllByTool` + `approvedAllByServer` 两个 Set；`a` 选项弹子菜单 |
| `src/main/java/com/yicli/hitl/ApprovalResult.java` | 新增 `APPROVED_ALL_BY_SERVER` 枚举值 + `isApprovedAllForServer()` 判断 |
| `src/main/java/com/yicli/hitl/HitlToolRegistry.java` | `executeTool` 进入审批前先查两个 approvedAll 集合 |
| `src/main/java/com/yicli/mcp/protocol/McpCallToolResult.java` | image fallback 文案微调：`"[此工具返回了 image。如果用户没明确要看图，优先用 take_snapshot 工具拿 DOM 文本快照]"` |
| `src/main/java/com/yicli/agent/Agent.java` 系统提示词 | 加「web_fetch vs 浏览器 MCP」决策表 + take_snapshot 优先指引 |
| `src/main/java/com/yicli/agent/PlanExecuteAgent.java` 同上 | 同 |
| `src/main/java/com/yicli/agent/SubAgent.java` 同上 | 同 |
| `src/main/java/com/yicli/cli/Main.java` | Banner v12 → v13；标语 → `Browser-Capable Agent CLI`；启动 hint 加一条「输入 '/mcp restart chrome-devtools' 重启浏览器 server」 |
...
- `.env.example`：新增 `YICLI_MCP_INITIALIZE_TIMEOUT_SECONDS` 示例
...
LLM 应该调 take_screenshot，但 YiCLI 把 image fallback 给 LLM 后，LLM 应该输出"已为您截图，但当前模型无法直接查看图片，请向我描述要从图中找什么信息"或类似友好提示。
...
**产出**：
- `Main.java`：启动时检测 `~/.yicli/mcp.json`，不存在则用模板创建（含 chrome-devtools）
- `McpServerManager.startAll`：另起 status printer 线程；每 5s 打印未就绪 server 等待时长
- `McpClient.initialize`：超时从 30s 提到 60s，可被 `yicli.mcp.initialize.timeout.seconds` 系统属性覆盖
...
- `.env.example` 加 `YICLI_MCP_INITIALIZE_TIMEOUT_SECONDS` 示例
...
1. ✅ 启动 YiCLI，看到 chrome-devtools 启动进度提示，最终 ready
...
- [ ] §5 列出的所有现有文件都按要求修改
- [ ] `~/.yicli/mcp.json` 不存在时启动自动创建（含 chrome-devtools）
