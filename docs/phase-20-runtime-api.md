...
- 生命周期：
  ...
  - `failed`
  - `canceled`
- Worker Pool：默认 2 个后台 worker，可用 `YICLI_TASK_WORKERS` 或 `-Dyicli.task.workers` 覆盖
...
实现位于 `src/main/java/com/yicli/runtime/api/`，使用 JDK 内置 `HttpServer`，不引入 Spring / Javalin。

启动：

```bash
YICLI_RUNTIME_API_KEY=your_local_api_key \
java -jar target/yicli-1.0-SNAPSHOT.jar serve --http --port 8080
```

安全策略：

- 仅监听 `127.0.0.1`
- 必须配置 `YICLI_RUNTIME_API_KEY` 或 `-Dyicli.runtime.api.key`
- 请求头支持：
...
YICLI_RUNTIME_API_KEY=test java -jar target/yicli-1.0-SNAPSHOT.jar serve --http --port 0
```
