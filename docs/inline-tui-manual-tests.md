...
```bash
cd /Users/itwanger/Documents/GitHub/yicli
mvn -q clean package -DskipTests
```

产物：`target/yicli-1.0-SNAPSHOT.jar`，所有用例都基于这个 jar 启动。
...
**预期**：
- Banner 显示简洁的 `YiCLI v16.1.0` 开屏，不带右侧盒线边框
...
```bash
YICLI_RENDERER=lanterna java -jar target/yicli-1.0-SNAPSHOT.jar
```
...
**预期**：
- stderr 出现 `⚠️ YICLI_TUI=true 已废弃，请改用 YICLI_RENDERER=lanterna`
...
```bash
YICLI_RENDERER=plain java -jar target/yicli-1.0-SNAPSHOT.jar
```
...
```bash
NO_COLOR=1 java -jar target/yicli-1.0-SNAPSHOT.jar
```
...
```bash
YICLI_RENDERER=weird java -jar target/yicli-1.0-SNAPSHOT.jar
```

**预期**：stderr `⚠️ 未识别的 YICLI_RENDERER='weird'，回退到 inline`，正常进 inline 模式。
...
```bash
TERM=dumb java -jar target/yicli-1.0-SNAPSHOT.jar
```
...
```
*
 YiCLI  idle  glm-5.1  ctx 0/200.0k  HITL OFF  MCP 4/4  Skill 2/2
...
**输入**：
```
把 ROADMAP.md 第一行改成 "# YiCLI 路线图（v16.1 测试）"
```
...
│   [6] 配置文件: ~/.yicli/config.json (只读视图，编辑请用编辑器)
...
## 23. YICLI_NO_STATUSBAR 禁用状态栏

```bash
YICLI_NO_STATUSBAR=true java -jar target/yicli-1.0-SNAPSHOT.jar
```
...
- 旧 Windows cmd / PuTTY 等不适合 ANSI 光标控制的终端：用 `YICLI_NO_STATUSBAR=true` 即可
