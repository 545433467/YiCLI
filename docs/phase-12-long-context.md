...
> 本期目标是让 YiCLI 的运行策略随模型上下文窗口变化，而不是继续使用固定 300K token 预算和固定 RAG topK。
...
- `AgentBudget` 动态预算：
  ...
  - 仍可用 `-Dyicli.react.token.budget=...` 覆盖
- Memory 策略：
  - short / balanced 保留压缩
...
- 不改变 `pom.xml` 的 Maven 产物版本，Jar 仍是 `yicli-1.0-SNAPSHOT.jar`

## 3. 核心文件

```text
src/main/java/com/yicli/context/
├── ContextMode.java
├── ContextProfile.java
