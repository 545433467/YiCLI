...
> Lanterna 三栏 TUI 作为可切换形态保留，通过 `YICLI_RENDERER=lanterna` 启用。
...
> 4. `src/main/java/com/yicli/cli/Main.java`（当前 CLI 入口，TUI 将在此之上做可视化层）
> 5. `src/main/java/com/yicli/util/TerminalMarkdownRenderer.java`（现有 Markdown 渲染器，TUI 需复用或增强）
> 6. `src/main/java/com/yicli/agent/Agent.java`、`PlanExecuteAgent.java`、`SubAgent.java`（系统提示词构造路径）
>
...
让 YiCLI 从"纯 CLI"升级为"终端 GUI"（TUI），具备文件树浏览、代码高亮、对话历史可视化、配置管理等产品级体验，**不牺牲任何现有 Agent 能力**。

**为什么做 TUI 而不是继续纯 CLI**：

- YiCLI 当前已内置 9 个工具 + MCP 60+ 个工具，用户记忆成本高
- 文件树 / 代码高亮 / 对话历史回滚这些"展示型"需求，纯 CLI 手工渲染成本高且体验差
- Claude Code、Cursor、Aider 等竞品都有 TUI，YiCLI 需要产品化竞争力
...
**不破坏现有命令行行为**：默认仍使用纯 CLI（JLine 行编辑器模式）。Lanterna 全屏 TUI 只在显式设置 `YICLI_TUI=true` 或 `-Dyicli.tui=true` 时启用；如果 `NO_TUI=true`、终端不可用或终端尺寸过小（`< 80×24`），仍降级为 CLI。
...
`shouldUseTui()` 判断：
1. 未设置 `YICLI_TUI=true` 且未设置 `-Dyicli.tui=true` → 保持默认 CLI
2. `NO_TUI=true` → 降级
...
**高亮引擎**：**不使用第三方高亮库**（如 Pygments / highlight.js 的 Java 移植版），而是用 YiCLI 自己第 4 期 `CodeChunker` 和 `CodeAnalyzer` 的能力做**轻量级语法着色**。
...
**为什么不用第三方库**：
- YiCLI 已有 AST 解析能力（第 4 期 JavaParser），Java 代码着色可以复用
...
**配置读写**：
- 读：`YiCliConfig.load()` 已有
- 写：`YiCliConfig.save(config)` 新增，写入 `~/.yicli/config.json`
...
**主题持久化**：`~/.yicli/config.json` 的 `theme` 字段（`"dark"` / `"light"`）
...
### 3.2 `~/.yicli/filetree-ignore.txt`（用户级忽略规则）
...
# ========== 第 16 期：TUI 产品化 ==========
# TUI 开关
# YICLI_TUI=true       # 显式启用 Lanterna 全屏 TUI；默认保持 CLI
# NO_TUI=true           # 强制 CLI，覆盖 YICLI_TUI=true
# YICLI_THEME=dark     # TUI 主题: dark / light（默认 dark）

# TUI 窗口尺寸（Lanterna 自适应，无需手动设置）
# YICLI_TUI_FILE_TREE_WIDTH=25%  # 文件树宽度（百分比或绝对列数）
# YICLI_TUI_FONT_SIZE=1          # 字体缩放（0=小, 1=中, 2=大）
```
...
### 4.1 新增类（`com.yicli.tui` 包）

| 文件 | 职责 |
|---|---|
| `src/main/java/com/yicli/tui/TuiBootstrap.java` | TUI 入口，判断 `shouldUseTui()`，创建 `LanternaWindow` 和布局 |
| `src/main/java/com/yicli/tui/LanternaWindow.java` | 封装 Lanterna `Window`，统一处理主题 / 大小变化 / 输入分发 |
| `src/main/java/com/yicli/tui/pane/LeftPane.java` | 左侧文件树面板（`FileTreePane` 类，基于 Lanterna `TreeWidget` 或自定义 `ListWidget`） |
| `src/main/java/com/yicli/tui/pane/CenterPane.java` | 中央对话流面板（`ChatPane`，基于 Lanterna `TextArea`，支持 Markdown 渲染 + 代码高亮 + 消息分色） |
| `src/main/java/com/yicli/tui/pane/RightPane.java` | 右侧状态栏面板（`StatusPane`，显示模型 / Token / 耗时 / 命令提示） |
| `src/main/java/com/yicli/tui/pane/InputBar.java` | 底部输入栏（`InputBar`，基于 Lanterna `TextField`，处理 Enter / Tab / ↑↓ / Ctrl 快捷键） |
| `src/main/java/com/yicli/tui/highlight/CodeHighlighter.java` | 轻量级语法高亮器（基于 `CodeChunker` 语言识别 + 正则词法着色，输出 ANSI 256 色字符串） |
| `src/main/java/com/yicli/tui/highlight/SyntaxTheme.java` | 主题定义（深色 / 浅色），映射语言关键字 / 字符串 / 注释到 Lanterna `TextColor` |
| `src/main/java/com/yicli/tui/history/ConversationHistoryManager.java` | 对话历史管理（按 `ConversationSnapshot` 组织，支持保存 / 恢复 / 删除） |
| `src/main/java/com/yicli/tui/history/ConversationStore.java` | 对话历史持久化（JSONL 格式，按天分文件 `~/.yicli/history/conversations-YYYY-MM-DD.jsonl`） |
| `src/main/java/com/yicli/tui/config/ConfigEditor.java` | 配置管理面板（`/config` 命令触发，支持 API Key / 模型 / 上下文模式 / HITL / Skill 启用状态编辑） |
| `src/main/java/com/yicli/tui/theme/ThemeManager.java` | 主题管理器（深色 / 浅色切换，持久化到 `config.json`） |
| `src/main/java/com/yicli/tui/theme/ColorPalette.java` | Catppuccin Mocha / Latte 调色板（背景 / 前景 / 关键字 / 字符串 / 注释等） |

### 4.2 修改类

| 文件 | 改动 |
|---|---|
| `src/main/java/com/yicli/cli/Main.java` | TUI 入口判断（`shouldUseTui()`）；降级时保留现有 CLI；TUI 模式下用 `TuiBootstrap.launch()` 替换 JLine `LineReader` 主循环；Banner v16.0.0 |
| `src/main/java/com/yicli/cli/CliCommandParser.java` | 新增 `CONFIG` 命令类型 + 子命令（`/config` 开配置面板）；TUI 模式下 `/clear` 清空对话流；`/exit` / `/quit` 关闭 TUI 窗口 |
| `src/main/java/com/yicli/util/TerminalMarkdownRenderer.java` | 增强 Markdown 渲染器：支持代码块折叠 + 语法高亮输出（调用 `CodeHighlighter`）；保持 CLI 模式向后兼容 |
| `src/main/java/com/yicli/config/YiCliConfig.java` | 新增 `tui` 配置段（`fileTree` / `editor`）；新增 `ThemeManager` 读取 / 写入 `theme` 字段 |
| `src/main/java/com/yicli/agent/Agent.java` | 流式输出时 TUI 模式改调用 `ChatPane.appendStreamingMessage()`（直接向面板写流式文本，不经过 `System.out`）；TUI 下的 HITL 弹窗调用 `TuiHitlDialog.show()` 替代 `TerminalHitlHandler` |
| `src/main/java/com/yicli/hitl/TerminalHitlHandler.java` | 新增 `TuiHitlDialog` 内部类（Lanterna 模态框弹窗），保留 CLI 模式兼容；`requestApproval` 根据 `LanternaWindow.isTuiMode()` 分发到 TUI 或 CLI |
| `src/main/java/com/yicli/agent/PlanExecuteAgent.java` | 同上：流式输出和 HITL 弹窗根据模式分发 |
| `src/main/java/com/yicli/agent/SubAgent.java` | 同上：Multi-Agent 流式输出分发 |
...
**配置路径**：
1. `YiCliConfig` 初始化时读 `config.json` 的 `tui.theme` 字段
2. `ThemeManager` 维护当前主题（`"dark"` 默认）
3. `/config` 编辑主题后调用 `YiCliConfig.save(config)` 写回
4. 下次启动时读取
...
```bash
# 终端尺寸 ≥ 80×24，显式启用 TUI
YICLI_TUI=true java -jar target/yicli-1.0-SNAPSHOT.jar
```
...
**场景 C：CLI 强制降级**
```bash
YICLI_TUI=true NO_TUI=true java -jar target/yicli-1.0-SNAPSHOT.jar
```
...
3. 用 `↑` / `↓` 移动选中 `Agent.java`，按 `Enter` 插入 `@file://src/main/java/com/yicli/agent/Agent.java`
4. Agent 收到引用后调 `read_file` 并展示内容

### 7.3 代码高亮

```
> 帮我看下 src/main/java/com/yicli/agent/Agent.java 里的 run() 方法做了什么
```
**期望**：
...
6. **文件树的 `Files.walk` 性能**：大仓库（如 `yicli` 自己，src/ 下 100+ 文件）`Files.walk` 可能 100ms+。解决：只懒加载一层（展开目录时 `Files.list`），不做预扫描。
...
10. **降级 CLI 模式的 TUI 专属功能降级行为**：`/config` 在 CLI 模式下应该降级为打印配置值（`YiCliConfig.toString()`），不弹出配置面板（因为 CLI 没有面板能力）。
...
| TUI vs CLI 关系 | **CLI 为主，TUI opt-in**（`YICLI_TUI=true` 或 `-Dyicli.tui=true` 才启动全屏界面） |
| 文件树 Git 状态 | **不做**（留后续期） |
...
| 主题持久化路径 | `~/.yicli/config.json` 的 `theme` 字段 |
| 对话历史持久化 | `~/.yicli/history/conversations-YYYY-MM-DD.jsonl`（按天，JSONL） |
...
**测试**：
- `TuiBootstrapTest`：未显式设置 `YICLI_TUI=true` → `shouldUseTui()` 返回 `false`
- `TuiBootstrapTest`：`YICLI_TUI=true` 且 `NO_TUI=true` → `shouldUseTui()` 返回 `false`
...
- **安装包分发**（参见 §6.7）：
  - `mvn clean package` 产出 `target/yicli-1.0-SNAPSHOT.jar`（pom.xml 仍保持 `1.0-SNAPSHOT`，Banner 显示 `16.0.0`）
...
- [x] `com.yicli.tui` 包落地，`Main.java` 接入 TUI / CLI 分支
- [x] `shouldUseTui()` 默认返回 CLI；显式 `YICLI_TUI=true` / `-Dyicli.tui=true` 才检查 TUI 条件；`NO_TUI=true` 和小终端降级
...
- [x] 对话历史写入 `~/.yicli/history/session_*.jsonl`
...
| TUI vs CLI | **CLI 为主，TUI opt-in**（`YICLI_TUI=true` 或 `-Dyicli.tui=true`） |
| 文件树 Git 状态 | **不做**（留后续期） |
| TUI 内嵌编辑器 | **不做** |
| 多窗口 / 分屏 | **本期不做** |
| 主题 | **深色 / 浅色两种**，Catppuccin 调色板，`config.json` 持久化 |
| 对话历史 | **新设计 `ConversationSnapshot`**，`~/.yicli/history/` JSONL 持久化 |
...
| 文件树忽略规则 | `~/.yicli/filetree-ignore.txt`（每行一个 glob），不存在用内置默认 |
...
- `~/.yicli/config.json` 已存在（第 15 期已有）
- `YICLI_TUI=true`
- 终端尺寸 ≥ 80×24
...
```
> 帮我看下 src/main/java/com/yicli/agent/Agent.java 里的 Agent 类主要做了什么
```
...
**期望行为序列**：
1. 未显式设置 `YICLI_TUI=true` → `shouldUseTui()` 返回 `false`
2. 不弹 Lanterna 全屏窗口
