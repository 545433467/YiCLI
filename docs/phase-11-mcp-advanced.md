...
如果 MCP server 在 `initialize` 返回的 capabilities 中声明 `resources`，YiCLI 会：
...
用户输入 `/cancel` 后，YiCLI 会设置当前运行的 `CancellationToken`，并中断后台 runner。ReAct、Plan-and-Execute、Multi-Agent 编排、工具批量执行以及 `execute_command` 会在边界处检查取消信号；如果底层 LLM HTTP 流式调用无法立即响应 Java interrupt，取消属于 best-effort，但后续工具执行不会继续推进。
