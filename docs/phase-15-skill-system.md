...
> 3. `src/main/java/com/yicli/agent/Agent.java`、`PlanExecuteAgent.java`、`SubAgent.java`（系统提示词构造路径）
> 4. `src/main/java/com/yicli/tool/ToolRegistry.java`（本期新增内置工具 `load_skill` 的注册位置）
> 5. `src/main/java/com/yicli/cli/Main.java` / `CliCommandParser.java`（本期新增 `/skill` 命令组）
...
让 YiCLI 拥有自己的 Skill 体系。Skill 是一个目录（含 `SKILL.md` + 可选 `references/` + 可选 `scripts/`），其中：
...
- Skill 启用状态持久化：`~/.yicli/skills.json` 存 disabled 列表（默认全启用）
...
| 加载顺序 | jar 内置 < 用户级 `~/.yicli/skills/` < 项目级 `<project>/.yicli/skills/`（同名后者覆盖前者） |
| 启用控制 | 默认启用；`~/.yicli/skills.json` 的 `disabled` 列表关掉 |
| 系统提示词占位 | 启用后只把 `name` + `description` 进 system prompt（"轻量索引"） |
| 完整指令展开 | LLM 调 `load_skill("name")` 后，YiCLI 把 SKILL.md 正文（去 frontmatter）拼到 next user message 的开头 |
...
- user message 是每轮重发，注入 body 会随轮次自然衰减（YiCLI 控制只保留最近 1 轮的注入，避免堆积）
...
2. **用户级**：`~/.yicli/skills/<name>/SKILL.md`
3. **项目级**：`<project>/.yicli/skills/<name>/SKILL.md`
...
- **不直接返回正文给 LLM 当工具结果**，而是：
  ...
  - 把 body 写入 `SkillContextBuffer`（per-session，per-agent-role），下一轮 user message 末尾被 YiCLI 拼接
...
**不做**：
- **不**复制 `~/.claude/skills/web-access/scripts/cdp-proxy.mjs`（那是 568 行的 HTTP 代理，YiCLI 第 13 / 14 期已经有 chrome-devtools MCP + shared 模式，能力等价，重写一个反而引入维护负担）
...
📚 Skills 加载（3 个）...
   ...
   ✓ yicli-internal user      description 124 字符
...
#   2. 用户级：~/.yicli/skills/<name>/SKILL.md
#   3. 项目级：<project>/.yicli/skills/<name>/SKILL.md
# 禁用列表持久化：~/.yicli/skills.json（{"disabled": ["name1", ...]}）
...
注意：`references/` 内的文件在 jar 内是只读 resources，LLM 通过 `read_file` 读取时需要走特殊路径——本期约定 **`references/` 文件由用户级或项目级 skill 提供**，jar 内置 skill 的 references 在启动期解压到 `~/.yicli/skills-cache/<name>/references/`，后续 LLM 用绝对路径读取。

解压策略：
- 启动期检测 `~/.yicli/skills-cache/<name>/.version` 与 jar 内置版本号是否一致
...
| 文件 | 职责 |
|---|---|
| `src/main/java/com/yicli/skill/Skill.java` | record，含 name / description / version / author / tags / source(builtin/user/project) / bodyPath / cachedReferencesDir |
| `src/main/java/com/yicli/skill/SkillFrontmatterParser.java` | 极简 YAML 子集解析（§3.2） |
| `src/main/java/com/yicli/skill/SkillRegistry.java` | 单例（由 Main 注入），扫描三层目录、合并、过滤 disabled、对外提供 `enabledSkills()` / `findSkill(name)` / `reload()` |
| `src/main/java/com/yicli/skill/SkillIndexFormatter.java` | 把 enabled skills 渲染成 system prompt 索引段（§3.8 模板，含 token 预算硬上限） |
| `src/main/java/com/yicli/skill/SkillContextBuffer.java` | per-session 内存 buffer（§3.6），提供 push(name, body) / drain() |
| `src/main/java/com/yicli/skill/SkillBuiltinExtractor.java` | 启动期把 jar 内 `resources/skills/<name>/references/` 解压到 `~/.yicli/skills-cache/<name>/`（§4.3） |
| `src/main/java/com/yicli/skill/SkillStateStore.java` | 读写 `~/.yicli/skills.json` |
| `src/main/java/com/yicli/skill/LoadSkillTool.java` | 内置工具实现，调用时往 SkillContextBuffer 写入 |

### 5.2 修改类

| 文件 | 改动 |
|---|---|
| `src/main/java/com/yicli/cli/Main.java` | 启动期初始化 SkillBuiltinExtractor + SkillRegistry + SkillContextBuffer；Banner v15.0.0；启动 hint 加 `/skill list` 提示；`/skill` 命令分发 |
| `src/main/java/com/yicli/cli/CliCommandParser.java` | 新增 `SKILL` 命令类型 + 子命令 `list` / `show <name>` / `on <name>` / `off <name>` / `reload` |
| `src/main/java/com/yicli/tool/ToolRegistry.java` | 注册 `load_skill` 工具；构造时接收 SkillRegistry + SkillContextBuffer 引用 |
| `src/main/java/com/yicli/agent/Agent.java` | system prompt 加 §3.8 skill 索引段；构造 user message 时调 `SkillContextBuffer.drain()` 拼接到原内容前 |
| `src/main/java/com/yicli/agent/PlanExecuteAgent.java` | 同上 |
| `src/main/java/com/yicli/agent/SubAgent.java` | 同上（三种角色都共享同一索引；buffer 在主 Agent 与子 Agent 间不共享，子 Agent 有自己的 buffer 实例） |
| `src/main/java/com/yicli/agent/AgentOrchestrator.java` | 在创建 SubAgent 时为每个 SubAgent 构造独立 SkillContextBuffer |
...
📚 Skills（3 个）
  ...
  ● yicli-internal  user      v0.2.0  YiCLI 项目内部约定与文档导航...
...
📖 Skill: web-access (builtin, v1.0.0)
  ...
  references/: ~/.yicli/skills-cache/web-access/references/ (6 个文件)
...
<SKILL.md body>
---
用户输入：（YiCLI 不重复用户原话，但 LLM 已有上下文）
...
⏸️ 已禁用 skill: verbose-debug
   下一轮 LLM 调用生效；已写入 ~/.yicli/skills.json
```
...
- 用户级 `~/.yicli/skills/web-access/SKILL.md`（v9.9.9，description 改成"用户版"）
- 不放项目级
...
再加项目级 `<project>/.yicli/skills/web-access/SKILL.md`（v0.0.1）：
...
2. YiCLI 输出 `已加载 skill 'web-access'...`
...
启用两个 skill：web-access 和 yicli-internal。让 LLM 完成跨域任务：

```
> 看下 YiCLI 项目的 ROADMAP.md，再去网上搜一下 LangGraph4J 最新版本
```

**期望**：LLM 加载 yicli-internal 处理 ROADMAP，加载 web-access 处理搜索；两个 skill body 都进 user message（按调用顺序拼接）。
...
清空 `~/.yicli/skills-cache/`，启动 YiCLI：

**期望**：自动重建 `~/.yicli/skills-cache/web-access/references/`，6 个 site-patterns 文件齐全，`.version` 文件存在。
...
4. **SkillContextBuffer 与流式输出竞态**：LLM 流式返回 tool_calls 时，YiCLI 还没 flush 完上一轮 reasoning，就收到新一轮 user message——buffer drain 时机要在**新 user message 构造时**而不是上一轮 LLM 完成时，避免提前清空。
5. **三层目录覆盖语义**：用户级 SKILL.md 必须**完整覆盖**内置版（不是字段级 merge），避免出现"用户改了 description 但 references 还指向 builtin 缓存路径"的诡异状态。
6. **builtin extractor 与多用户**：如果系统多用户共用 `~/.yicli`，extractor 写入要带文件锁。本期接受单用户假设，文档明示。
...
- [ ] SkillBuiltinExtractor 启动期解压 references/ 到 `~/.yicli/skills-cache/`
...
| 启用状态持久化 | `~/.yicli/skills.json` 的 disabled 列表 |
