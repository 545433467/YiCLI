...
> 3. 现有的 `src/main/java/com/yicli/tool/ToolRegistry.java`、`src/main/java/com/yicli/hitl/HitlToolRegistry.java`、`src/main/java/com/yicli/policy/AuditLog.java` —— 你要集成进这三处
>
> **执行原则**：
...
让 YiCLI 接入 MCP 生态：stdio 子进程 server 与 Streamable HTTP 远程 server 都能用，工具自动注册到 `ToolRegistry`，与 HITL / AuditLog 协同。**默认开启**。

最终交付：

- `com.yicli.mcp` 包（4 个子包，约 15 个类、~1500 行代码）
...
测试 mirror 这个结构放到 `src/test/java/com/yicli/mcp/`。
...
- 用户级 `~/.yicli/mcp.json` + 项目级 `.yicli/mcp.json`，**项目级覆盖用户级**（按 server 名 merge）
...
`com.yicli.hitl.ApprovalPolicy.requiresApproval` 改为：
...
`com.yicli.tool.ToolRegistry.AUDIT_TOOLS` 扩展为「危险工具集 + 任何 `mcp__` 前缀工具」。审计字段保持不变；`tool` 字段就是 `mcp__filesystem__read_file`，过滤时按前缀。
...
**新增**：
- `src/main/java/com/yicli/mcp/` 整个包
- `src/test/java/com/yicli/mcp/` 测试

**修改**：
- `src/main/java/com/yicli/tool/ToolRegistry.java`：新增 `registerMcpTool(McpToolDescriptor)` / `unregisterMcpTool(String toolName)`，`executeTool` 路由到 `McpToolBridge`；`AUDIT_TOOLS` 判断扩展为含 `mcp__` 前缀
- `src/main/java/com/yicli/hitl/ApprovalPolicy.java`：`requiresApproval` 加 `mcp__` 前缀判断
- `src/main/java/com/yicli/hitl/TerminalHitlHandler.java`：弹窗加 server 信息行
- `src/main/java/com/yicli/policy/AuditLog.java`：新增 args 脱敏（regex 替换 token / Bearer / authorization）
- `src/main/java/com/yicli/cli/Main.java`：启动时 `McpServerManager.startAll`；shutdown hook；CLI 命令分支；启动 banner 后打印 MCP server 状态
- `src/main/java/com/yicli/cli/CliCommandParser.java`：5 个新 `MCP_*` 命令枚举与 parse 分支
- `src/main/java/com/yicli/agent/Agent.java`：系统提示词加 MCP 工具说明
- `src/main/java/com/yicli/agent/PlanExecuteAgent.java`：同上
- `src/main/java/com/yicli/agent/SubAgent.java`：同上
...
**不**实现 `/mcp add` / `/mcp remove`：配置编辑通过文件 + 重启 YiCLI。
...
- 环境变量缺失 → **报错**，不静默保留
- server 启动失败 → **不阻塞** YiCLI 启动，标 ERROR 跳过
- 工具描述截断长度：1000 字符
- MCP 工具调用日志：写到 `~/.yicli/logs/mcp/<server>.log`（按天滚动，复用 logback 风格但用单独 logger）
...
| 配置文件位置 | `~/.yicli/mcp.json`（用户级）+ `.yicli/mcp.json`（项目级，可入 git） |
