...
- `src/main/java/com/yicli/lsp/LspManager.java`
  - 持有项目根路径
...
```bash
# 默认开启
YICLI_LSP_ENABLED=true

# 默认 20
YICLI_LSP_MAX_DIAGNOSTICS=20
```

系统属性同样支持：

```bash
-Dyicli.lsp.enabled=false
-Dyicli.lsp.max.diagnostics=20
```
...
- 不解析 `execute_command` 里的手写补丁，也不支持 `edit_file` / `apply_patch` 工具，因为当前 YiCLI 内置工具里还没有这两个工具。
