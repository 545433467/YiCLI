...
新建 `com.yicli.render` 包，把 Agent / HITL / Main 对终端的全部输出收口到一个接口：

```java
package com.yicli.render;
...
public interface Renderer extends AutoCloseable {
    ...
    // ---- 配置面板 / 命令选择 ----
    void openConfigPanel(YiCliConfig config);
...
└── RendererFactory.java                 根据 YICLI_RENDERER 创建实例

com.yicli.render.inline/                ← 新增（inline 实现）
...
com.yicli.render.plain/                 ← 新增（纯 println 降级）
└── PlainRenderer.java                   等价 phase-15 行为，无折叠、无状态栏

com.yicli.tui/                          ← 保留（Lanterna 实现）
...
`com.yicli.tui.highlight.CodeHighlighter` 两个实现都用，不迁移。

### 4.3 `RendererFactory` 选型

```java
public final class RendererFactory {
    public static Renderer create(Terminal terminal, YiCliConfig config) {
        String mode = resolveMode();  // env: YICLI_RENDERER
        return switch (mode) {
    ...
    private static String resolveMode() {
        String prop = System.getProperty("yicli.renderer");
        if (prop != null && !prop.isBlank()) return prop.toLowerCase();
        String env = System.getenv("YICLI_RENDERER");
...
| `YICLI_RENDERER=lanterna` | Lanterna 全屏 TUI |
| `YICLI_RENDERER=plain` | 纯 println，无折叠、无状态栏 |
| `YICLI_RENDERER=inline` | 显式声明 inline |
| `NO_COLOR=1` | 已有：禁用 ANSI 颜色，对三种模式都生效 |
| `YICLI_TUI=true` | 兼容旧版：等价于 `YICLI_RENDERER=lanterna`，打 deprecation 提示 |

### 4.4 修改的现有文件

| 文件 | 改动 |
|---|---|
| `com.yicli.cli.Main` | 启动时 `Renderer renderer = RendererFactory.create(...)`；删除 `TuiBootstrap.shouldUseTui` 二分支判断；CLI 主循环全部经 `renderer` 调用，不再直接 `System.out.println` |
| `com.yicli.agent.Agent$StreamRenderer` | 持有 `Renderer`；`onReasoningDelta`/`onContentDelta` 走 `renderer.appendReasoningChunk/appendContentChunk`；`appendToolCall` 走 `renderer.startToolCall(...).appendResult(...)` |
| `com.yicli.agent.PlanExecuteAgent`、`SubAgent` | 同上 |
| `com.yicli.hitl.TerminalHitlHandler` | 改名 `RendererHitlHandler`，持有 `Renderer`；`requestApproval` 直接调 `renderer.promptApproval(req)`，不区分 CLI / TUI |
| `com.yicli.hitl.SwitchableHitlHandler` | 简化：只剩开关逻辑（enable/disable），delegate 永远是同一个 `RendererHitlHandler` |
| `com.yicli.hitl.TuiHitlHandler` | 删除（被 `LanternaRenderer.promptApproval` 取代） |
| `com.yicli.tui.TuiBootstrap` | 删除 `shouldUseTui()` / `launch()`；保留为空类或直接删除（`LanternaRenderer` 自己负责启动 Lanterna 主循环） |
| `com.yicli.tui.TuiSessionController` | 删除（职责由 `Main` + `LanternaRenderer` 承担） |
| `com.yicli.util.AnsiStyle` | 扩充 cursor save/restore、line clear、scroll region、reverse 等序列；底层常量挪到 `com.yicli.render.inline.AnsiSeq` |
...
**全部保留**：`com.yicli.tui.*` 在新架构下作为 Lanterna 实现存在，不删代码。
...
- `README.md`：新增"渲染器形态切换"段，说明 `YICLI_RENDERER` 三档
- `ROADMAP.md`：第 16 期标 ✅，下一步 phase-17 为 LSP 诊断注入，图片输入后移 phase-21
- `.env.example`：把 `YICLI_TUI=true` 标 deprecated，新增 `YICLI_RENDERER=inline|lanterna|plain`
...
- 新建 `com.yicli.render` 包：`Renderer` / `ToolBlock` / `StatusInfo` / `ApprovalRequest` / `ApprovalDecision` / `PaletteItem` / `PaletteResult`
...
- Banner 微调："YiCLI v16.1.0"（小版本号区分形态修正）

---

## 8. 端到端手测清单（Day 6 必跑，inline / lanterna 双形态）

每条用例在 `YICLI_RENDERER=inline`（默认）和 `YICLI_RENDERER=lanterna` 各跑一遍：

1. ✅ 启动 YiCLI，状态栏/状态面板显示模型 + token
...
8. ✅ `YICLI_RENDERER=plain`：纯 println，无折叠、无状态栏
9. ✅ 终端 resize：inline 状态栏重新定位 + 已折叠块标 frozen；lanterna widget 自适应
10. ✅ Vim 模式 `YICLI_VI=true`：输入框支持 hjkl 移动
11. ✅ 200 行代码粘贴：格式保留，不触发提交
12. ✅ Lanterna 启动失败模拟：自动回退 inline，打日志提示
13. ✅ `YICLI_TUI=true`（deprecated 兼容）：等价 lanterna，提示 deprecation
...
| 默认形态 | **inline** |
| 切换机制 | **启动时**通过 `YICLI_RENDERER` 选择，不支持运行时热切换 |
| 接口位置 | `com.yicli.render` 包，与 inline / lanterna 实现解耦 |
...
| 旧 `YICLI_TUI=true` | 兼容映射到 `YICLI_RENDERER=lanterna`，打 deprecation |
...
- [ ] `com.yicli.render` 接口包落地（`Renderer` + 全部数据载体）
- [ ] `com.yicli.render.inline` 落地，`InlineRenderer` 通过所有单测
- [ ] `com.yicli.render.plain.PlainRenderer` 落地
- [ ] `com.yicli.tui.LanternaRenderer` 落地，`TuiBootstrap` / `TuiSessionController` / `TuiHitlHandler` 删除
- [ ] `RendererFactory` 三档切换工作正常，`YICLI_TUI=true` 兼容映射
