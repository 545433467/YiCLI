...
> 3. `src/main/java/com/yicli/mcp/McpServerManager.java`（restart / disable / enable 已就绪，本期要补「运行时改 args 后重启」）
> 4. `src/main/java/com/yicli/hitl/`（`approvedAllByTool` / `approvedAllByServer` 已就绪，本期要补「敏感页面强制单步审批」）
> 5. `src/main/java/com/yicli/policy/`（`PathGuard` / `CommandGuard` / `AuditLog` 已就绪，本期新增 `BrowserGuard`、`SensitivePagePolicy`，`AuditEntry` 加可选 `metadata` 字段）
>
> **核心原则**：第 13 期已经把 `chrome-devtools-mcp` 接进来跑通；本期**不写 CDP 协议**——chrome-devtools-mcp 原生支持 `--autoConnect` 连接 Chrome 144+ 的 `chrome://inspect/#remote-debugging` 实例，也支持旧式 `--browser-url=http://127.0.0.1:9222`，YiCLI 这边只做「运行时切换 server 启动参数 + 登录态安全约束 + UX 引导」。
...
- 敏感页面识别（默认规则 + 用户级 `~/.yicli/sensitive_patterns.txt`）+ 命中后强制单步审批（绕过 `approvedAllByServer`）
- `close_page` 工具硬保护：只允许关闭 YiCLI 当次会话 `new_page` 出来的 tab，否则抛 `PolicyException`
...
- 跨设备 / 跨 YiCLI 实例的 session 同步
- Cookie 持久化到 YiCLI 自己的存储
...
**默认行为不变**：`~/.yicli/mcp.json` 不存在时仍生成 `--isolated=true`（向后兼容第 13 期用户）。`browser_connect` / `/browser connect` 是运行期动作，不主动改用户 mcp.json 文件，**只在内存中切换 args**；用户重启 YiCLI 后回到 isolated。这样：
- 默认安全（isolated 仍是默认）
- 运行时按需 connect
- 用户的 mcp.json 不被 YiCLI 偷偷改动

如果用户希望默认就走 shared，文档里明示：自己改 `~/.yicli/mcp.json` 把 args 改成 `["-y", "chrome-devtools-mcp@latest", "--autoConnect"]`。旧式端口可用 `["-y", "chrome-devtools-mcp@latest", "--browser-url=http://127.0.0.1:9222"]`。
...
- 默认内置 glob 列表：
  ...
  - `*://*/2fa*`
  - `*://*/password*`
- 用户级追加：`~/.yicli/sensitive_patterns.txt`，每行一个 glob（`#` 开头注释、空行忽略）。文件不存在视为空
...
- `close_page` 调用前 `BrowserGuard.check()` 校验：目标 pageId 不在 `agentOpenedTabs` 里 → 抛 `PolicyException("close_page 拒绝：该 tab 不是 YiCLI 本次会话开启的，不允许关闭。如需关闭请用户手动操作。")`
...
**isolated 模式下的行为**：tabs 都是 YiCLI 自己开的，正常允许 close。判定：
- isolated 模式 → 跳过该硬保护
- shared 模式 → 启用
...
**启动期**：YiCLI 启动时，如果 `mcp.json` 里 `chrome-devtools` 配置已经是 `--browser-url=...`（用户自己改的），不在启动期探活——`chrome-devtools-mcp` 自己会失败并写 stderr，`/mcp logs chrome-devtools` 可见。启动期间不阻塞别的 server。
...
**保持第 13 期不变**：`~/.yicli/mcp.json` 不存在时仍生成 `--isolated=true`。本期不修改默认模板。理由见 §3.1。
...
# 用户级敏感页面规则：~/.yicli/sensitive_patterns.txt（每行一个 glob，# 开头注释）
# 默认敏感规则：*/settings* */admin* */billing* */account* */security* */oauth* */auth* */2fa* */password*
```

### 4.3 用户级敏感规则文件

`~/.yicli/sensitive_patterns.txt`（**YiCLI 不主动创建**，只读取）：
...
| 文件 | 职责 |
|---|---|
| `src/main/java/com/yicli/browser/BrowserMode.java` | 枚举 `ISOLATED` / `SHARED` |
| `src/main/java/com/yicli/browser/BrowserSession.java` | 单例。当前 `mode` / `targetHost` / `targetPort` / `lastNavigatedUrl` / `agentOpenedTabs`。提供 `switchMode(...)`、`recordNavigated(url)`、`recordOpenedTab(tabId)`、`isAgentOpened(tabId)` |
| `src/main/java/com/yicli/browser/BrowserConnectivityCheck.java` | `probe(host, port)` → `Probe(connected, browserVersion, errorMessage)` |
| `src/main/java/com/yicli/policy/BrowserGuard.java` | `check(toolName, argsJson)` → `BrowserCheckResult(blocked, reason, requiresPerCallApproval, metadata)`；从 `BrowserSession` + `SensitivePagePolicy` 取依赖 |
| `src/main/java/com/yicli/policy/SensitivePagePolicy.java` | 加载默认 + 用户级 glob，`isSensitive(url)` 返回 `(matched: bool, pattern: String)` |

### 5.2 修改类

| 文件 | 改动 |
|---|---|
| `src/main/java/com/yicli/mcp/McpServerManager.java` | 新增 `restartWithArgs(name, newArgs)` 方法 |
| `src/main/java/com/yicli/mcp/config/McpServerConfig.java` | `args` 增加 setter（包可见或 public，文档注释「运行时切换专用」） |
| `src/main/java/com/yicli/policy/AuditLog.java` | `AuditEntry` 加可选 `metadata` 字段，新增带 metadata 的静态工厂；反序列化老格式容错 |
| `src/main/java/com/yicli/tool/ToolRegistry.java` | `executeTool` 入口对 `mcp__chrome-devtools__*` 工具加一道 `BrowserGuard.check`；`blocked=true` 抛 `PolicyException`；非 blocked 把 metadata 透传给 audit |
| `src/main/java/com/yicli/hitl/HitlToolRegistry.java` | 进入审批前先查 `BrowserGuard.requiresPerCallApproval`，命中则强制单步审批 |
| `src/main/java/com/yicli/hitl/TerminalHitlHandler.java` | 审批弹窗在敏感页面命中时多打印一行 `⚠️ 敏感页面，需单独确认` + matched pattern + URL；不展示「[a] 全部放行」 |
| `src/main/java/com/yicli/hitl/ApprovalRequest.java` | 新增可选字段 `String sensitiveNotice`（敏感命中时由调用方填，弹窗显示）；保持兼容老构造 |
| `src/main/java/com/yicli/cli/Main.java` | 新增 `/browser` 命令分发（参考 `/mcp` 实现） |
| `src/main/java/com/yicli/cli/CliCommandParser.java` | 增加 `BROWSER` 命令解析及子命令 `status` / `connect [port]` / `disconnect` / `tabs` |
| `src/main/java/com/yicli/agent/Agent.java` 系统提示词 | §5.4 段落 |
| `src/main/java/com/yicli/agent/PlanExecuteAgent.java` 同上 | 同 |
| `src/main/java/com/yicli/agent/SubAgent.java` 同上 | 同 |
| `src/main/java/com/yicli/cli/Main.java` Banner | `VERSION = "14.0.0"`；标语 `Session-Aware Browser Agent CLI` |
...
- YiCLI 已对敏感 URL（settings / admin / billing / account / security / oauth 等）启用强制单步审批，每个改写型工具调用都会弹窗
...
🛡️ 策略拒绝: close_page 拒绝：该 tab 不是 YiCLI 本次会话开启的，
   不允许关闭。如需关闭请用户手动操作。

[Agent 转告用户]
我无法关闭你原本打开的 GitHub tab，YiCLI 只允许关闭它自己开的 tab。
你可以在 Chrome 里直接关掉，或者告诉我关 YiCLI 自己开过的哪个 tab。
```
...
```
> /browser connect
[/browser tabs 显示用户已开 7 个 tab，YiCLI 自开 0 个]
> 帮我关掉第一个 tab
```
...
写入 `~/.yicli/sensitive_patterns.txt`：
...
3. **`close_page` 的 pageId 解析**：chrome-devtools-mcp 工具返回结构 YiCLI 这边没强约定，需要在 Day 1 实际跑一次 `new_page` 看看 result 长什么样，再决定怎么解析 pageId。如果发现完全没法解析，**保守**：shared 模式下一律拒绝 `close_page`，让用户手动关。
4. **敏感规则的 URL 来源**：`navigate_page` 的 args 显式带 url，好办；但 `click` / `fill` 这些 args 里没 url，靠 `BrowserSession.lastNavigatedUrl` 缓存。如果 Agent 用 `evaluate_script` 改 location 跳转 → YiCLI 抓不到。**接受这个边界**，文档明示「敏感判定基于最近一次 navigate_page；evaluate_script 跳转无法识别」。
...
9. **跨平台路径**：`~/.yicli/sensitive_patterns.txt` 在 Windows 是 `%USERPROFILE%\.yicli\sensitive_patterns.txt`。沿用项目既有约定 `Path.of(System.getProperty("user.home"), ".yicli", ...)` 不要硬编码 `/`。
