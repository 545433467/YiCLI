...
1. 准备测试图片，建议放在 `~/yicli-image-cases/` 下：
...
预期：
- 自动写入 `~/.yicli/cache/clip-<ts>.png`
- note 行 `[已附加图片: 剪贴板 (clip-<ts>.png), mimeType=image/png, bytes=…]`
- LLM 回复明显基于图像内容

### Case 18 — Ctrl+V 注入

光标停在 YiCLI 输入框，先复制好图（同 Case 17），然后按 **Ctrl+V**（不是 Cmd+V，Cmd+V 会被终端拦截成本地粘贴文本）。
...
通过 `ssh user@host` 进入远程主机，启动 YiCLI，然后：
